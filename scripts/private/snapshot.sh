...
SNAP_DIR=angular-recipecat-snapshots
...
do
  ...
  if [[ $i = 0 ]]; then
    ...
    rm -rf $SNAP_DIR/step-$i/app/recipes
  fi

  if [[ $i = 5 ]]; then
    cp -r app/img $SNAP_DIR/step-0/app/
    cp -r app/recipes $SNAP_DIR/step-0/app/
  fi
done
...
zip -r angular-recipecat-`date +"%y%m%d_%H%M"` $SNAP_DIR
